...
public class CameraScript_V3 : MonoBehaviour
{
    float Yrot;
    RaycastHit hit;
    GameObject grabOBJ;
    ...
    void Update()
    {
        ...
        if(grabOBJ != null && grabOBJ.GetComponent<IsGun>() == null)approach();
    }
    void porte()
    {
        // bouton test porte
    ...
    void approach()
    {
        if (grabOBJ != null)
        {
            ...
            {
                if(grabPos.localPosition.z < 2)
                {
                    grabPos.transform.Translate(Vector3.forward * 120 * Time.deltaTime);
                }
            }
            else if (Input.GetAxis("Mouse ScrollWheel") < 0f)
            {
                if(grabPos.localPosition.z > 0.5)
                {
                    grabPos.transform.Translate(Vector3.forward * -120f * Time.deltaTime);
                }
            }
        }
    }
